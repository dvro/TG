...
\label{alg:sgp1}
\begin{algorithmic}[1]
\REQUIRE {$prototypes$: uma lista para os representantes}
\REQUIRE {$groups$: uma lista de grupos de instâncias}
	\FORALL {classe $C$}
		\STATE $G$ = $\bigcup$ instâncias da classe $C$
		\STATE $Adicione(G, groups)$
		\STATE $Adicione(Centroide(G),prototypes)$ 
	\ENDFOR
	\STATE $count = 1$
	\WHILE {$count \ne 0$}
		\STATE $count$ = $Quantidade(groups)$
		\FORALL {$G$ em $groups$}
			\FORALL {$e_i$ em $group$}
				\STATE Calcule $1NN(e_i, prototypes)$
			\ENDFOR
			\IF {$\forall$ $e_i$ em $G$ o protótipo mais próximo é o representante do grupo}
				\STATE $count$ = $count - 1$
			\ELSIF {$\forall$ $e_i$ em $G$ a classe do protótipo mais próximo for diferente da do protótipo centróide} 
				\STATE trace um  hiperplano que passa pelo centróide de $G$ e cujo vetor normal é a primeira componente principal.
				\STATE divida $G$ em dois grupos, as instâncias que estão acima, e abaixo do hiperplano traçado.
				\STATE calcule o centróide de cada grupo criado e atualize $G$ e $prototypes$.
			\ELSIF {$\exists$ $e_i$ em $G$ tal que o protótipo mais próximo é o representante de outro grupo mas possui a mesma classes}
				\STATE remova todo $e_i$ e inclua-os no grupo do protótipo mais próximo.
				\STATE atualize os protótipos de cada grupo.
			\ELSIF {$\exists$ $e_i$ em $G$ tal que o protótipo mais próximo é o representante de outro grupo mas possui classe diferente}
				\STATE remova todo $e_i$ de $G$ e crie um novo grupo.
				\STATE atualize $prototypes$ e $groups$
			\ENDIF
		\ENDFOR
	\ENDWHILE
	\RETURN  $prototypes$
