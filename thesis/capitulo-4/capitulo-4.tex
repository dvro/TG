...
Por sua vez o CNN, de alto poder de redução, reduziu demais a classe minoritária (ver figura \ref{fig:iris2}), mas como foi utilizado o 3-NN, obteve taxa de acerto média de 0.00\% para esta classe (ver figura \ref{fig:iris1}), enquanto a marjoritária obteve 100\% de acerto.
...
O OSS, técnica utilizada para bases desbalanceadas, obteve uma taxa de acerto total de 84.18\% em média, uma taxa de acerto baixa em relação as outras técnicas. A redução também não foi tão alta, a quantidade de protótipos resultantes foi em média 34.18\% da baso original. Porém, como o \textit{One-Sided Selection} dá prioridade a classe minoritária, pode-se observar que a taxa de acerto desta classe foi de 100\%, o que torna esta uma técnica eficiente quando o caso de interesse é a classe minoritária.
...
Nesta base, com baixa sobreposição de classes, o ENN e o Tomek Links foram inúteis, pois não apresentaram redução de dados. Já o CNN se mostrou altamente ineficiente, pois reduziu tanto a classe minoritária (para uma instâncias) que tornou a taxa de acerto desta classe nula, pois foi utilizado K=3.
...
Outra técnica apropriada para este tipo de base foi o \textit{One-Sided Selection}. Por priorizar a classe minoritária, o OSS garante uma boa taxa de acerto para esta classe. Porém, comparando esta técnica como o SGP e LVQ, percebe-se que ela tem um baixo nível de redução, sendo recomendado a aplicação de outra técnica após o OSS.
...
\caption{Taxa de acerto da classe minoritária por técnica na base Glass}
\label{fig:glass1}
\end{figure}
...
\caption{Tamanho a base de dados após seleção de protótipos na base Glass}
\label{fig:glass2}
\end{figure}


Observando a tabela \ref{tab:glass}, percebe-se que, neste exemplo, além de a base ser mais desbalanceada, existe uma maior sobreposição de classes que o exemplo anterior. Em geral, as técnicas obtiveram uma boa taxa de acerto total, mas a taxa de acerto da classe minoritária variou muito de acordo com a técnica.
...
O OSS, obteve uma taxa de acerto total inferior as técnicas citadas anteriormente, sendo 85.53\% em média. Todavia, observa-se que a taxa de acerto da base minoritária foi de 60\%, a mesma taxa do KNN e, diferentemente do Tomek Links, a quantidade de protótipos selecionados pelo OSS foi em média apenas 10.67\% do conjunto de treinamento original. Isso mostra que, mesmo com um nível de desbalanceamento maior, \textit{One-Sided Selection} tem um alto poder de redução e prioriza a classe minoritária.

As versões do LVQ tiveram desempenho parecido, porém, o LVQ 3 se mostrou mais eficiente para a classe minoritária. Observando os experimentos do capítulo anterior, percebe-se que, neste experimento, o espalhamento controlado dos protótipos resultou em um aumento percentual da taxa de acerto da classe minoritária de 10\% em relação as outras versões do LVQ.

Comparando o LVQ 3 com as técnicas anteriores, percebe-se que ele teve um alto nível de redução da base, apenas 5.86\% de instâncias foram necessárias para ter a mesma taxa de acerto da classe minoritária que o próprio KNN.
...
O \textit{Self-Generating Prototypes 1} foi altamente ineficiente em classificar a classe minoritária. A tabela \ref{tab:glass} mostra que o SGP 1 reduziu a base de dados para 3.17\% do conjunto de treinamento original, mas o custo disso foi uma taxa de acerto média de apenas 10\% para classe minoritária. No caso da Iris 0, como existia um baixo nível de desbalanceamento o SGP foi eficiente, porém, com uma base mais desbalanceada, esta técnica removeu a maioria das instâncias da classe minoritária, tornando os protótipos resultantes inviáveis para qualquer classificador.
...
Assim, conclui-se que, em bases com alto nível de desbalanceamento e com sobreposição de classes, as versões do SGP são inviáveis, principalmente quando o caso de interesse é a classe minoritária. Já o LVQ, mais uma vez se mostrou eficiente para identificação da classe minoritária, mostrando que a eliminação do desbalanceamento é uma estratégia interessante para este caso.

Da mesma forma que o experimento anterior, o OSS teve um bom desempenho para classificar instâncias da classe minoritária, sendo recomendado mesmo com alto nível de desbalanceamento.
...
A base Yeast 6 é de mais alto nível de desbalanceamento e será o referencial para casos extremos neste trabalho.
